...
if (in_array('artisan', $idv_roles, true)) {
    $idv_rdv_url = home_url('/dashboard/artisan/rdv/');
} elseif (in_array('client', $idv_roles, true)) {
    $idv_rdv_url = home_url('/mes-rdv/');
} else {
    $idv_rdv_url = home_url('/professionnels/');
}
...
$idv_items = [
    ...
    ['__rdv__', $idv_rdv_url, 'RDV', '<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>'],
];
?>
<nav class="idv-bottomnav" aria-label="Navigation rapide">
  <?php foreach ($idv_items as [$idv_match, $idv_url, $idv_label, $idv_svg]) :
      $idv_active = $idv_match === '__rdv__'

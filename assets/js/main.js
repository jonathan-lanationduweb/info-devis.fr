...
if (devisForm) {
  devisForm.addEventListener('submit', async (e) => {
    ...
    const msg = document.getElementById('devis-result');
    btn.disabled = true;
    btn.textContent = 'Envoi en cours...';
    if (msg) msg.className = '';

    try {
      const res  = await fetch(devisForm.action, { method: 'POST', body: new FormData(devisForm) });
      const data = await res.json();
      if (data.success) {
        window.location.href = data.redirect || '/devis/confirmation';
      } else {
        if (msg) {
          msg.className = 'alert alert-danger';
          msg.textContent = (data.errors || ['Une erreur est survenue']).join(' ');
        }
        btn.disabled = false;
        btn.textContent = 'Envoyer ma demande';
      }
    } catch {
      if (msg) { msg.className = 'alert alert-danger'; msg.textContent = 'Erreur réseau. Réessayez.'; }
      btn.disabled = false;
      btn.textContent = 'Envoyer ma demande';
    }
  });

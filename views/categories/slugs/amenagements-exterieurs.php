<?php /* views/categories/slugs/amenagements-exterieurs.php */ ?>
<main class="pt-20">
  ...
  <div class="relative z-10 max-w-[1440px] mx-auto px-8 w-full">
    <div class="max-w-2xl">
      ...
      <p class="text-xl font-body text-on-surface-variant mb-10 max-w-lg leading-relaxed">
        <?= Security::e($category['description'] ?? 'Des solutions architecturales pour sublimer vos espaces de vie exterieurs en havres de paix.') ?>
      </p>
      <div class="flex items-baseline gap-4 mb-10">
        <span class="text-stone-400 font-label text-sm uppercase tracking-widest">A partir de</span>
        <span class="text-4xl font-headline text-primary italic">
          <?= !empty($category['prix_min']) ? number_format($category['prix_min'],0,',',' ').' &euro;' : '300 &euro;' ?>
        </span>
      </div>
      <div class="flex flex-wrap gap-4">
        <a href="<?= APP_URL ?>/devis?categorie=<?= $category['id'] ?>" class="bg-primary text-on-primary px-10 py-5 rounded-xl font-label tracking-widest uppercase text-xs hover:opacity-90 transition-all shadow-2xl shadow-primary/20">Explorer les solutions</a>
        <a href="<?= APP_URL ?>/tarifs-pro" class="border border-outline-variant/30 text-on-surface px-10 py-5 rounded-xl font-label tracking-widest uppercase text-xs hover:bg-surface-container-low transition-all">Consulter les tarifs</a>
      </div>
    </div>
  </div>
</section>
...
<section class="py-24 px-8 max-w-[1440px] mx-auto">
  <div class="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
    <div class="max-w-xl">
      ...
      <p class="text-on-surface-variant leading-relaxed text-lg">Decourvrez nos realisations architecturales, de la terrasse en essence rare a la veranda bioclimatique.</p>
    </div>
    <a href="<?= APP_URL ?>/categories" class="font-label text-xs uppercase tracking-widest text-primary border-b border-primary/20 pb-1 hover:border-primary transition-all">Voir tout le catalogue</a>
  </div>
  <div class="grid grid-cols-1 md:grid-cols-12 gap-6" style="min-height:600px">
    <div class="md:col-span-8 relative overflow-hidden rounded-xl group cursor-pointer" style="min-height:400px">
      <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80" alt="Terrasse">
      <div class="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors"></div>
      <div class="absolute bottom-10 left-10 text-white">
        <p class="font-label text-xs tracking-widest uppercase mb-2 opacity-80">Architecture Bois</p>
        <h3 class="font-headline text-3xl">Terrasses de Prestige</h3>
      </div>
    </div>
    <div class="md:col-span-4 relative overflow-hidden rounded-xl group cursor-pointer" style="min-height:400px">
      <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src="https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=800&q=80" alt="Jardin">
      <div class="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors"></div>
      <div class="absolute bottom-10 left-10 text-white">
        <p class="font-label text-xs tracking-widest uppercase mb-2 opacity-80">Transparence</p>
        <h3 class="font-headline text-2xl">Jardins &amp; Verandas</h3>
      </div>
    </div>
    <div class="md:col-span-4 relative overflow-hidden rounded-xl group cursor-pointer" style="min-height:300px">
      <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=800&q=80" alt="Pergola">
      <div class="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors"></div>
      <div class="absolute bottom-10 left-10 text-white">
        <p class="font-label text-xs tracking-widest uppercase mb-2 opacity-80">Ombre &amp; Lumiere</p>
        <h3 class="font-headline text-2xl">Pergolas Innovantes</h3>
      </div>
    </div>
    <div class="md:col-span-8 relative overflow-hidden rounded-xl group cursor-pointer" style="min-height:300px">
      <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src="https://images.unsplash.com/photo-1590856029826-c7a73142bbf1?w=800&q=80" alt="Piscine">
      <div class="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors"></div>
      <div class="absolute bottom-10 left-10 text-white">
        <p class="font-label text-xs tracking-widest uppercase mb-2 opacity-80">Aquatique</p>
        <h3 class="font-headline text-2xl">Bassins &amp; Piscines</h3>
      </div>
    </div>
  </div>
</section>

<!-- Services BDD ou statiques -->
<section class="bg-surface-container-low py-24 px-8">
  <div class="max-w-[1440px] mx-auto">
    ...
    <?php
    $svcs = !empty($services) ? $services : array_map(function($s){ return ['nom'=>$s,'description'=>'Nos artisans interviennent avec la plus grande precision.']; }, ['Terrasses de Prestige','Jardins et Verandas','Pergolas Innovantes','Bassins et Piscines']);
    ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-outline-variant/10 overflow-hidden rounded-3xl border border-outline-variant/10 shadow-sm">
      ...
      <div class="bg-surface p-10 hover:bg-surface-container-high transition-colors group">
        ...
        <?php if(!empty($svc['prix_min'])): ?>
        <p class="text-primary font-label text-xs font-bold uppercase tracking-widest mb-4"><?= number_format($svc['prix_min'],0,',',' ') ?> &ndash; <?= number_format($svc['prix_max'],0,',',' ') ?> &euro;</p>
        <?php endif; ?>
        <a href="<?= APP_URL ?>/devis?categorie=<?= $category['id'] ?>" class="text-primary font-label text-xs tracking-widest uppercase flex items-center gap-2 group-hover:gap-4 transition-all">En savoir plus &rarr;</a>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
    ...
    <div class="relative">
      ...
      <div class="space-y-8">
        ...
        <div class="flex gap-6 items-start">
          ...
          <div>
            <h4 class="font-headline text-xl mb-2"><?= $av[0] ?></h4>
            <p class="text-on-surface-variant text-sm leading-relaxed"><?= $av[1] ?></p>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
...
<section class="py-24 px-8 max-w-[1440px] mx-auto text-center">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-16 relative">
    ...
    <div class="relative">
      <div class="w-16 h-16 bg-primary text-on-primary rounded-full flex items-center justify-center mx-auto mb-8 font-headline text-2xl shadow-lg relative z-10"><?= $i+1 ?></div>
      <h3 class="font-headline text-2xl mb-4"><?= $s[0] ?></h3>
      <p class="text-on-surface-variant text-sm leading-relaxed"><?= $s[1] ?></p>
    </div>
    <?php endforeach; ?>
  </div>
</section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-2xl p-6 border border-outline-variant/20 hover:-translate-y-1 hover:shadow-lg transition-all">
        ...
        <div class="flex items-center gap-2 mb-3">
          <span class="text-yellow-400"><?= str_repeat('&#9733;',(int)round($a['rating_avg']??0)).str_repeat('&#9734;',5-(int)round($a['rating_avg']??0)) ?></span>
          <span class="text-sm font-semibold"><?= number_format((float)($a['rating_avg']??0),1) ?></span>
        </div>
...
<section class="py-16 px-8 bg-surface-container-low">
  <div class="max-w-3xl mx-auto">
    <h2 class="font-headline text-3xl text-center mb-10">Questions frequentes</h2>
    <div class="space-y-4">
      <?php foreach($faq as $i=>$item): ?>
      <div class="bg-white rounded-xl border border-outline-variant/20 overflow-hidden">
        <button onclick="toggleFaq(<?=$i?>)" class="w-full text-left px-6 py-5 flex justify-between items-center font-semibold text-sm font-body bg-transparent border-none cursor-pointer">
          <?= Security::e($item['q']) ?>
          <span id="faq-icon-<?=$i?>" class="text-primary text-xl flex-shrink-0">+</span>
        </button>
...
<section class="py-16 px-8">
  <div class="max-w-[1440px] mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <?php foreach($relatedCats as $rel): ?>
      <a href="<?= APP_URL ?>/categorie/<?= Security::e($rel['slug']) ?>" class="group bg-surface-container-low border border-outline-variant/20 rounded-xl p-6 text-center hover:border-primary hover:-translate-y-1 hover:shadow-md transition-all">
        <h3 class="font-headline text-lg mb-1 group-hover:text-primary transition-colors"><?= Security::e($rel['name']) ?></h3>
        <?php if(!empty($rel['prix_min'])): ?>
        <p class="text-xs text-primary font-semibold">A partir de <?= number_format($rel['prix_min'],0,',',' ') ?> &euro;</p>
        <?php endif; ?>
      </a>
      <?php endforeach; ?>
    </div>
  ...
  <div class="max-w-[1440px] mx-auto bg-primary rounded-[2rem] p-16 md:p-24 relative overflow-hidden text-center text-on-primary">
    ...
    <div class="relative z-10 max-w-2xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        <a href="<?= APP_URL ?>/devis?categorie=<?= $category['id'] ?>" class="bg-white text-primary px-12 py-5 rounded-lg font-label tracking-widest uppercase text-sm hover:bg-primary-container transition-all shadow-2xl shadow-black/10">Obtenir une etude gratuite</a>
        <a href="<?= APP_URL ?>/tarifs-pro" class="border border-white/30 text-white px-12 py-5 rounded-lg font-label tracking-widest uppercase text-sm hover:bg-white/10 transition-all">Contacter un conseiller</a>
      </div>
    </div>
  </div>
</section>

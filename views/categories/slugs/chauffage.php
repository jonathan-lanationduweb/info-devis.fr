<?php /* views/categories/slugs/chauffage.php */ ?>
<main class="pt-20">

  <!-- Hero -->
  <section class="relative min-h-[870px] flex items-center overflow-hidden bg-surface-container-low">
    <div class="absolute inset-0 z-0">
      <img class="w-full h-full object-cover" src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1400&q=80" alt="Chauffage">
    ...
    <div class="relative z-10 max-w-[1440px] mx-auto px-8 w-full">
      <div class="max-w-2xl">
        <span class="font-label tracking-[0.2em] uppercase text-xs text-primary mb-6 block font-semibold">Excellence Thermique</span>
        <h1 class="font-headline text-6xl md:text-8xl text-on-surface leading-tight mb-8">Chauffage</h1>
        <p class="text-xl font-body text-on-surface-variant mb-10 max-w-lg leading-relaxed"><?= Security::e($category['description'] ?? 'L\'art de maîtriser le climat intérieur. Des solutions d\'ingénierie avancées pour un confort absolu.') ?></p>
        <div class="flex items-baseline gap-4 mb-10">
          <span class="text-stone-400 font-label text-sm uppercase tracking-widest">À partir de</span>
          <span class="text-4xl font-headline text-primary italic"><?= !empty($category['prix_min']) ? number_format($category['prix_min'], 0, ',', ' ') . ' €' : '800 €' ?></span>
        </div>
  ...
  <section class="py-24 px-8 bg-surface-container-low">
    <div class="max-w-[1440px] mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6" style="height:500px">
        <div class="md:col-span-8 relative overflow-hidden rounded-xl group"><img src="https://images.unsplash.com/photo-1581235720704-06d3acfcb36f?w=800&q=80" alt="Réalisation" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
          <div class="absolute bottom-8 left-8 text-white">
            <h3 class="font-headline text-2xl italic">Chauffage — Finitions premium</h3>
          </div>
        </div>
  ...
  <section class="py-24 px-8 bg-surface">
    <div class="max-w-[1440px] mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-outline-variant/10 overflow-hidden rounded-3xl border border-outline-variant/10 shadow-sm">
        ...
        }, ['Chaudieres a Condensation', 'Pompes a Chaleur', 'Planchers Rayonnants', 'Radiateurs Design']);
        foreach ($svcs as $svc): ?>

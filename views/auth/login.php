  ...
  <div class="w-full max-w-md">
    ...
    <?php if (!empty($error)): ?>
      <div class="mb-6 p-4 border-l-4 border-red-400 bg-red-50">
        <p class="text-sm text-red-700 font-body"><?= Security::e($error) ?></p>
      </div>
    <?php endif; ?>
    ...
    <form action="<?= APP_URL ?>/connexion" method="POST" class="space-y-8 bg-white p-8 shadow-sm border border-gray-100">
      <?= Security::csrfField() ?>
      ...
      <div class="space-y-2">
        ...
        <input class="w-full bg-surface-container border-none py-4 px-4 text-on-surface focus:ring-0 focus:border-b-2 focus:border-primary transition-all font-body"
          id="email" name="email" type="email" placeholder="votre@email.fr" required
      ...
      <div class="space-y-2">
        <div class="flex justify-between items-center mb-2">
          ...
          <a class="text-[10px] uppercase tracking-wider text-primary/70 font-bold hover:text-primary transition-colors"
            href="<?= APP_URL ?>/mot-de-passe-oublie">Oublié ?</a>
        </div>
        <div class="relative">
    ...
    <div class="mt-8 text-center space-y-4">
      <p class="text-sm text-on-surface-variant font-body">
        ...
        <a class="text-primary font-bold ml-1 hover:underline underline-offset-4"
          href="<?= APP_URL ?>/inscription">S'inscrire gratuitement</a>
      </p>
      <p class="text-sm text-on-surface-variant font-body">
        Vous êtes artisan ?
        <a class="text-primary font-bold ml-1 hover:underline underline-offset-4"
          href="<?= APP_URL ?>/inscription?type=artisan">Créer un compte Pro →</a>
      </p>
    </div>

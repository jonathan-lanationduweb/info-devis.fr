...
class AuthController extends BaseController {
    ...
    public function login(): void {
        ...
        switch ($user['role']) {
            ...
            case 'artisan': $this->redirect('/dashboard/artisan'); break;
            default:        $this->redirect('/dashboard/client'); break;
        }
    }
    ...
    private function registerArtisan(int $userId): void {
        $siret       = preg_replace('/\s/', '', $this->input('siret', ''));
        ...
        // Vérification SIRET en ligne
        $siretVerified = SiretService::verify($siret);

        $artisanId = Database::insert('artisans', [
            'user_id'         => $userId,
            'siret'           => $siret,
            'siret_verified'  => $siretVerified ? 1 : 0,
